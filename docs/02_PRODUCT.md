# Lumina Product

## Product Positioning

Lumina is a beauty discovery and trust marketplace.

The goal is not to replace the tools artists already use, such as Instagram, Square, GlossGenius, Fresha, or Booksy. Instead, Lumina helps clients discover beauty professionals earlier in their decision process, compare their work more clearly, and book with confidence.

Lumina helps artists get discovered for the quality of their work, not just their price.

## Core Customer Insight

Beauty professionals are facing more competition from salons, booth renters, independent artists, and at-home providers. Many feel pressure to discount because clients often compare by price before understanding the difference in quality.

Clients usually care about more than price. They want work that is beautiful, long-lasting, trendy, comfortable, friendly, and on time.

Lumina should help make those trust signals visible.

## What Lumina Should Emphasize

- Visual portfolio quality
- Clear services and pricing
- Reviews and trust signals
- Response time
- Artist personality and professionalism
- Booking confidence
- Comparison between artists
- Saved collections for beauty planning

## What Lumina Is Not Trying To Be In V1

Lumina is not trying to replace full salon management platforms.

V1 should avoid:

- POS systems
- Payroll
- Inventory management
- Employee scheduling
- Card readers
- Advanced marketing automation
- Full calendar infrastructure

Artists can continue using their own booking platforms while Lumina focuses on discovery, trust, communication, and request flow.

## Elevator Pitch

Lumina is a beauty marketplace that helps clients discover trustworthy beauty professionals through verified portfolios, transparent information, and modern communication.

Instead of scrolling through Instagram, Facebook, or TikTok hoping to find the right artist, clients can confidently discover, compare, message, and eventually book—all in one place.

---

# The Problem

Clients struggle to:

- Find trustworthy beauty professionals
- Compare artists fairly
- Know real pricing
- See verified work
- Contact artists easily
- Keep conversations organized

Beauty professionals struggle to:

- Get discovered
- Build trust with new clients
- Manage inquiries across multiple apps
- Showcase their portfolio professionally

---

# Our Solution

Lumina brings everything into one platform.

Clients can:

- Browse artists
- Compare artists
- Save favorites
- Message professionals
- Request appointments
- Build confidence before booking

Professionals can:

- Build a professional profile
- Showcase verified work
- Respond to clients
- Grow their business

---

# Target Audience

### Clients

People looking for:

- Lash artists
- Nail technicians
- Hairstylists
- Estheticians
- Brow artists
- Makeup artists
- Beauty services

---

### Professionals

Independent beauty professionals who want more exposure and a better way to manage clients.

---

# Long-Term Vision

Become the most trusted beauty marketplace in the industry.