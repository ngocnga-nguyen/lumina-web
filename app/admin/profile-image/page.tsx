"use client";

import { useEffect, useState } from "react";
import { supabase } from "@/lib/supabase";

type Artist = {
  id: string;
  name: string;
};

export default function ProfileImageAdmin() {
  const [artists, setArtists] = useState<Artist[]>([]);
  const [artistId, setArtistId] = useState("");
  const [imageUrl, setImageUrl] = useState("");
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const fetchArtists = async () => {
      const { data } = await supabase
        .from("artists")
        .select("id,name")
        .order("name");

      setArtists(data || []);
    };

    fetchArtists();
  }, []);

  const handleSave = async () => {
    if (!artistId || !imageUrl) {
      alert("Choose artist + add image URL");
      return;
    }

    setLoading(true);

    const { error } = await supabase
      .from("artists")
      .update({
        profile_image_url: imageUrl,
      })
      .eq("id", artistId);

    setLoading(false);

    if (error) {
      alert(error.message);
      return;
    }

    alert("Profile image saved ✨");
    setImageUrl("");
  };

  return (
    <main className="min-h-screen bg-lumina-surface text-lumina-text">
      <section className="mx-auto w-full max-w-[1280px] px-5 py-10 md:px-8 md:py-14">
        <h1
          className="text-[38px] font-semibold leading-[1.05] md:text-[50px]"
          style={{ fontFamily: "Georgia, Times New Roman, serif" }}
        >
          Profile Image Admin
        </h1>

        <p className="mt-3 max-w-[680px] text-[14px] leading-[1.7] text-lumina-text-muted">
          Set artist face photo, logo, or brand image.
        </p>

        <div className="mt-8 max-w-[520px] space-y-4 rounded-[24px] border border-lumina-border bg-lumina-surface p-5 md:p-6">
        <select
          value={artistId}
          onChange={(e) => setArtistId(e.target.value)}
          className="w-full rounded-[14px] border border-lumina-border bg-lumina-surface px-4 py-3 text-[15px] outline-none transition focus:border-lumina-text-muted/60"
        >
          <option value="">Choose artist</option>

          {artists.map((artist) => (
            <option key={artist.id} value={artist.id}>
              {artist.name}
            </option>
          ))}
        </select>

        <input
          type="text"
          placeholder="Paste image URL"
          value={imageUrl}
          onChange={(e) => setImageUrl(e.target.value)}
          className="w-full rounded-[14px] border border-lumina-border bg-lumina-surface px-4 py-3 text-[15px] outline-none transition placeholder:text-lumina-text-muted/75 focus:border-lumina-text-muted/60"
        />

        {imageUrl && (
          <img
            src={imageUrl}
            alt="Preview"
            className="h-[260px] w-full rounded-[16px] object-cover"
          />
        )}

        <button
          onClick={handleSave}
          disabled={loading}
          className="rounded-full bg-lumina-black px-6 py-3 text-[14px] font-medium text-white transition hover:opacity-90 disabled:opacity-50"
        >
          {loading ? "Saving..." : "Save profile image"}
        </button>
        </div>
      </section>
    </main>
  );
}
